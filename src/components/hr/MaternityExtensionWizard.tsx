...
interface MaternityExtensionWizardProps {
    employeeId: string;
    employeeName?: string;
...
const STEPS = [
    { label: 'Параметры', description: 'Укажите период и больничный' },
    { label: 'Предпросмотр', description: 'Проверьте документы' },
    { label: 'Подписание', description: 'Отправьте на подпись' },
];

export function MaternityExtensionWizard({ employeeId, employeeName, existingDocuments = [], onClose, onSuccess }: MaternityExtensionWizardProps) {
    ...
    const validateStep = () => {
        if (currentStep === 0) {
            if (!params.vacationStart) {
                setError('Укажите дату начала продления');
                return false;
            }
            if (!params.vacationEnd) {
                setError('Укажите дату окончания продления');
                return false;
            }
            ...
            if (!params.sickLeaveNumber) {
                setError('Укажите номер больничного листа');
                return false;
            }
        }
        return true;
    ...
    const handleNext = async () => {
        ...
        if (currentStep === 0) {
            ...
            setError(null);
            try {
                const result = await api.generateProcessDocuments(employeeId, 'maternity_extension', {
                    vacationStart: params.vacationStart,
                    vacationEnd: params.vacationEnd,
                    sickLeaveNumber: params.sickLeaveNumber,
                    sickLeaveDate: params.sickLeaveDate || params.vacationStart,
                    sickLeaveSeries: params.sickLeaveSeries,
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">
                            Продление декрета {employeeName && <span className="text-slate-500 font-normal">— {employeeName}</span>}
                        </h2>
                        <p className="text-sm text-slate-500">Пакет документов для продления отпуска по беременности и родам</p>
                    </div>
                    ...
                    {currentStep === 0 && (
                        <div className="space-y-6 max-w-lg mx-auto">
                            <div className="text-center mb-6">
                                ...
                                <h3 className="text-base font-semibold text-slate-900">Параметры продления</h3>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="p-4 rounded-lg border bg-slate-50/50">
                                    <h4 className="text-sm font-semibold mb-3">Больничный лист</h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={params.sickLeaveNumber}
                                                onChange={e => handleParamChange('sickLeaveNumber', e.target.value)}
                                                placeholder="9794484"
                                                className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                            />
                                        </div>
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={params.sickLeaveSeries}
                                                onChange={e => handleParamChange('sickLeaveSeries', e.target.value)}
                                                placeholder="БД"
                                                className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                            />
                                        </div>
                                    ...
                                    <div className="mt-3">
                                        ...
                                        <input
                                            type="date"
                                            value={params.sickLeaveDate}
                                            onChange={e => handleParamChange('sickLeaveDate', e.target.value)}
                                            className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                        />
                                        <p className="text-xs text-slate-400 mt-1">Если не указать — совпадёт с датой начала продления</p>
                                    </div>
                                </div>
                            </div>
                        </div>
