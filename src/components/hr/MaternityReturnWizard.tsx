...
interface MaternityReturnWizardProps {
    employeeId: string;
    employeeName?: string;
...
const STEPS = [
    { label: 'Параметры', description: 'Укажите дату выхода' },
    { label: 'Предпросмотр', description: 'Проверьте документы' },
    { label: 'Подписание', description: 'Отправьте на подпись' },
];

export function MaternityReturnWizard({ employeeId, employeeName, existingDocuments = [], onClose, onSuccess }: MaternityReturnWizardProps) {
    ...
    const validateStep = () => {
        if (currentStep === 0) {
            if (!params.returnDate) {
                setError('Укажите дату выхода на работу');
                return false;
            }
        }
        return true;
    ...
    const handleNext = async () => {
        ...
        if (currentStep === 0) {
            ...
            setError(null);
            try {
                const result = await api.generateProcessDocuments(employeeId, 'maternity_return', {
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">
                            Выход с декрета {employeeName && <span className="text-slate-500 font-normal">— {employeeName}</span>}
                        </h2>
                        <p className="text-sm text-slate-500">Пакет документов для выхода на работу</p>
                    </div>
                    ...
                    {currentStep === 0 && (
                        <div className="space-y-6 max-w-lg mx-auto">
                            <div className="text-center mb-6">
                                ...
                                <h3 className="text-base font-semibold text-slate-900">Дата выхода на работу</h3>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700 mb-1.5">
                                        Дата выхода на работу <span className="text-red-500">*</span>
                                    </label>
                                    <input
                                        type="date"
