...
export function VacationWizard({ employeeId, employeeName, existingDocuments = [], onClose, onSuccess }: VacationWizardProps) {
    ...
    const handleNext = async () => {
        ...
        if (currentStep === 0) {
            ...
            setError(null);
            try {
                const result = await api.generateProcessDocuments(employeeId, 'vacation', {
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">
                            Отпуск {employeeName && <span className="text-slate-500 font-normal">— {employeeName}</span>}
                        </h2>
                        <p className="text-sm text-slate-500">Пакет документов на отпуск</p>
                    </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                    />
                                    <p className="text-xs text-slate-400 mt-1">Если не совпадает с периодом — укажите точное количество</p>
                                </div>
                            </div>
                        </div>
                    )}
