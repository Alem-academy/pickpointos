...
interface TerminationByEmployeeWizardProps {
    employeeId: string;
    employeeName?: string;
...
export function TerminationByEmployeeWizard({ employeeId, employeeName, existingDocuments = [], onClose, onSuccess }: TerminationByEmployeeWizardProps) {
    const {
        hasExistingDocs,
        initialStep,
        isLoadingContent,
    ...
    const handleNext = async () => {
        ...
        if (currentStep === 0) {
            const confirmed = window.confirm(
                'Будет сформирован пакет документов на увольнение по инициативе работника:\n• Заявление на увольнение\n• Приказ об увольнении\n\nРанее сгенерированные документы останутся в системе. Продолжить?'
            );
            if (!confirmed) return;
            ...
            setError(null);
            try {
                const result = await api.generateProcessDocuments(employeeId, 'termination_employee_initiative', {
    ...
    const handleSendToEmployee = async () => {
        ...
        try {
            const doc = successfulDocs.find(d => d.type === '18_zayavlenie-na-uvolnenie');
            if (!doc?.document?.id) {
                alert('Заявление сотрудника не найдено');
                return;
            }
    ...
    const handleEmployerSign = () => {
        const doc = successfulDocs.find(d => d.type === '19_prikaz-ob-uvolnenii');
        if (!doc?.document?.id || signedDocIds.includes(doc.document.id)) return;
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">
                            Увольнение по инициативе работника {employeeName && <span className="text-slate-500 font-normal">— {employeeName}</span>}
                        </h2>
                        <p className="text-sm text-slate-500">Пакет документов на увольнение по инициативе работника</p>
                    </div>
