...
export function MaternityLeaveWizard({ employeeId, employeeName, existingDocuments = [], onClose, onSuccess }: MaternityLeaveWizardProps) {
    ...
    const handleNext = async () => {
        ...
        if (currentStep === 0) {
            ...
            setError(null);
            try {
                const result = await api.generateProcessDocuments(employeeId, 'maternity_leave', {
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">
                            Отпуск по беременности {employeeName && <span className="text-slate-500 font-normal">— {employeeName}</span>}
                        </h2>
                        <p className="text-sm text-slate-500">Пакет документов на отпуск по беременности и родам</p>
                    </div>
                    ...
                    {currentStep === 0 && (
                        <div className="space-y-6 max-w-lg mx-auto">
                            <div className="text-center mb-6">
                                ...
                                <h3 className="text-base font-semibold text-slate-900">Параметры отпуска</h3>
                                ...
                                <div className="p-4 rounded-lg border bg-slate-50/50">
                                    ...
                                    <div className="mt-3">
                                        ...
                                        <input
                                            ...
                                            className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                        />
                                        <p className="text-xs text-slate-400 mt-1">Если не указать — совпадёт с датой начала отпуска</p>
                                    </div>
                                </div>
                            </div>
                        </div>
