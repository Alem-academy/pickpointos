...
interface NameChangeWizardProps {
    employeeId: string;
    employeeName?: string;
...
const STEPS = [
    { label: 'Параметры', description: 'Укажите причину изменения' },
    { label: 'Предпросмотр', description: 'Проверьте документы' },
    { label: 'Подписание', description: 'Отправьте на подпись' },
];

export function NameChangeWizard({ employeeId, employeeName, existingDocuments = [], onClose, onSuccess }: NameChangeWizardProps) {
    ...
    const [params, setParams] = useState({
        oldLastName: '',
        newLastName: '',
        changeReason: 'в связи с заключением брака',
        marriageCertNumber: '',
        marriageCertDate: '',
        marriageCertIssuer: '',
    });
    ...
    const validateStep = () => {
        if (currentStep === 0) {
            if (!params.oldLastName || !params.newLastName) {
                setError('Укажите старую и новую фамилию');
                return false;
            }
            if (!params.marriageCertNumber) {
                setError('Укажите номер свидетельства');
                return false;
            }
            if (!params.marriageCertDate) {
                setError('Укажите дату свидетельства');
                return false;
            }
        }
        return true;
    ...
    const handleNext = async () => {
        ...
        if (currentStep === 0) {
            const confirmed = window.confirm(
                'Будет сформирован пакет документов для изменения ФИО:\n• Заявление об изменении данных\n• Приказ об изменении ФИО\n• Доп. соглашение об изменении фамилии\n\nРанее сгенерированные документы останутся в системе. Продолжить?'
            );
            if (!confirmed) return;
            ...
            setError(null);
            try {
                const result = await api.generateProcessDocuments(employeeId, 'name_change', {
                    oldLastName: params.oldLastName,
                    newLastName: params.newLastName,
                    changeReason: params.changeReason,
                    marriageCertNumber: params.marriageCertNumber,
                    marriageCertDate: params.marriageCertDate,
                    marriageCertIssuerRu: params.marriageCertIssuer,
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
                    <div>
                        <h2 className="text-lg font-bold text-slate-900">
                            Изменение ФИО {employeeName && <span className="text-slate-500 font-normal">— {employeeName}</span>}
                        </h2>
                        <p className="text-sm text-slate-500">Пакет документов для изменения персональных данных</p>
                    </div>
                    ...
                    {currentStep === 0 && (
                        <div className="space-y-6 max-w-lg mx-auto">
                            <div className="text-center mb-6">
                                <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-orange-100 mb-3">
                                    <FileText className="h-6 w-6 text-orange-600" />
                                </div>
                                <h3 className="text-base font-semibold text-slate-900">Изменение фамилии</h3>
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1.5">
                                            Старая фамилия <span className="text-red-500">*</span>
                                        </label>
                                        <input
                                            type="text"
                                            value={params.oldLastName}
                                            onChange={e => handleParamChange('oldLastName', e.target.value)}
                                            placeholder="Иванова"
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1.5">
                                            Новая фамилия <span className="text-red-500">*</span>
                                        </label>
                                        <input
                                            type="text"
                                            value={params.newLastName}
                                            onChange={e => handleParamChange('newLastName', e.target.value)}
                                            placeholder="Петрова"
                                            className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                        />
                                    </div>
                                ...
                                <div>
                                    ...
                                    <input
                                        type="text"
                                        value={params.changeReason}
                                        onChange={e => handleParamChange('changeReason', e.target.value)}
                                        placeholder="в связи с заключением брака"
                                        className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                    />
                                </div>

                                <div className="p-4 rounded-lg border bg-slate-50/50">
                                    <h4 className="text-sm font-semibold mb-3">Свидетельство о браке</h4>
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <input
                                                type="text"
                                                value={params.marriageCertNumber}
                                                onChange={e => handleParamChange('marriageCertNumber', e.target.value)}
                                                placeholder="1234"
                                                className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                            />
                                        </div>
                                        <div>
                                            ...
                                            <input
                                                type="date"
                                                value={params.marriageCertDate}
                                                onChange={e => handleParamChange('marriageCertDate', e.target.value)}
                                                className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                            />
                                        </div>
                                    </div>
                                    <div className="mt-3">
                                        ...
                                        <input
                                            type="text"
                                            value={params.marriageCertIssuer}
                                            onChange={e => handleParamChange('marriageCertIssuer', e.target.value)}
                                            placeholder="Алматинский районный ЗАГС"
                                            className="w-full rounded-lg border border-slate-200 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-colors"
                                        />
                                    </div>
                                </div>
